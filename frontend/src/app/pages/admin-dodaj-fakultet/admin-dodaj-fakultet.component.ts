...
export class AdminDodajFakultetComponent {
    ...
    if(this.id){
      this.fakultetService.getById(this.id).subscribe((data)=>{
        this.form.patchValue({
          ...
          email: data.email,
          opis: data.opis,
          dekan_id: data.rektor.id,
          univerzitet_id: data.univerzitetId,
  ...
  onSubmit(){
    const payload: FakultetDTO = {
      ...
      rektor: this.nastavnici.find(n => n.id === +this.form.value.dekan_id)!,
        univerzitetId: this.form.value.univerzitet_id,
        adresa: {
        id: null,
